...
export default function SplashCursor({
  ...
  BACK_COLOR = { r: 0.0, g: 0.0, b: 0.0 }, // Changed back color
  TRANSPARENT = true
}: SplashCursorProps) {
  ...
  useEffect(() => {
    ...
    try {
      result = getWebGLContext(canvas);
      console.log('WebGL initialized successfully');
    } catch (error) {
      console.warn('WebGL not supported, disabling SplashCursor:', error);
      setWebGLSupported(false);
      return;
    }

    if (!result || !result.gl || !result.ext) {
      setWebGLSupported(false);
      return;
    }
    ...
    // @ts-ignore gl is checked to be not null
    ...
    function generateColor(): ColorRGB {
      // Modify to use Red hue: Hue is between start and end. Red is around 0.0 or 1.0 (0 to 360 deg)
      // We'll generate hues between 0.0 and 0.05 (red to slight orange-red) or 0.95 to 1.0 (pinkish-red to red)
      let hue = Math.random() * 0.1 - 0.05; // -0.05 to +0.05
      if (hue < 0) hue += 1.0;

      const c = HSVtoRGB(hue, 1.0, 1.0);
      c.r *= 0.5; // Boost the red influence to make it prominent
      c.g *= 0.15; // Lower green
      c.b *= 0.15; // Lower blue
      return c;
    }
